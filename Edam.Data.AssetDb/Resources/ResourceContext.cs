...
namespace Edam.Data.AssetManagement.Resources
{
      ...
      IResourceData
   {
            ...
            DataReferenceOption.DomainsTermsAndElements)
      {
         ...
         if (r.Success)
         {
            results.Instance = AssetData.ToDataElement(r.Elements);
            results.Succeeded();
         }
         else
         {
         ...
         string batchId, string domainUri, string versionId, string groupId)
      {
         ...
         if (t.Status == TaskStatus.RanToCompletion)
         {
            if (t.Result != null)
            {
               ...
               results.Instance = t.Result.ResponseData;
            }
         }
         t.Dispose();
         ...
         AssetType type)
      {
         ...
         try
         {
            ...
            bresults = BatchInsertUpdate(
               bresults.Instance, ns.NamePath.DomainUri, assets.VersionId, groupId);
         ...
         string typeEntityName)
      {
         ...
         if (f == null)
         {
            var elementCopy = element.DeepCopy<IDataElement>();
