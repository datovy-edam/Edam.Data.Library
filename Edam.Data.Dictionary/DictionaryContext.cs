   ...
   public class DictionaryContext : DbContext, IDictionaries
   {
         ...
         DictionaryType type = DictionaryType.Term)
      {
         ...
         switch(type)
         {
            case DictionaryType.Term:
               ...
               results.Instance = items.ToList<ITermInfo>();
               results.Succeeded();
               break;
            case DictionaryType.Word:
               var words = FindWordItem(term);
               results.Instance = words.ToList<ITermInfo>();
               results.Succeeded();
               break;
            case DictionaryType.Queue:
               var queueItems = FindQueueItem(term);
               results.Instance = queueItems.ToList<ITermInfo>();
               results.Succeeded();
               break;
            default:
               results.Failed(EventCode.ReferenceNotFound);
         ...
         DictionaryType type = DictionaryType.Term)
      {
         ...
         if (tryFinding)
         {
            ...
            if (results.Success && results.Instance.Count > 0)
            {
               item = results.Instance[0];
            }
         }
