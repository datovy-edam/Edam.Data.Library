...
namespace Edam.Data.Dictionary.Api
{
   ...
   public class FreeDictionaryApi : IDictionaryApi
   {
      ...
      public ResultsLog<ITermInfo?> LookUp(ITermInfo term)
      {
         ...
         if (results.Success && results.Instance != null)
         {
            results.Instance.KeyID = term.KeyID;
            results.Instance.Soundex = term.Soundex;
         }
         return results;
      }
         ...
         try
         {
            ...
            foreach (var term in terms)
            {
               ...
               if (results.Success)
               {
                  ...
                  {
                     if (results.Instance.Status == EntryStatus.Unavailable)
                     {
                        continue;
                     }

                     results.Instance.Status = EntryStatus.Found;
                     m_LookupResults.Found++;
                     m_LookupResults.ProcessedTerm = results.Instance;
                  }
                  else
                  {
